...
void Loja::mostrarItens()
{
    ...
    {
        if (par.second == true)
        { // se o item está disponível, ele vai aparecer na lista
            std::cout << (it->first) << " - " << (par.first.nome)
                      << " - R$ " << (par.first.preco) << "  "
                      << "(Disponíveis: )" << (par.first.quantidade) << std::endl;
        }
    }
}
...
void Loja::pedido(unsigned cod, unsigned qnt)
{
    ...
    for (; Iit != _itens.end(); ++Iit)
    {
        ...
        {
            if (it->first == cod)
            { // Se o item já existe no carrinho, só é adicionado a quantidade desejada
                _carrinho[cod].quantidade += qnt;
                _itens[cod].first.quantidade -= qnt;
                return;
            }
            else if ((Iit->first == cod) && (parIn.second == true) && (parIn.first.quantidade >= qnt))
            { // item é válido e a quantidade é suficiente
                //(remover)
            ...
            { // item é válido e a quantidade é insuficiente
            ...
            else if ((Iit->first == cod) && (parIn.second == false) && ((parIn.first.quantidade >= qnt) || (parIn.first.quantidade < qnt)))
            { // item inválido
...
void Loja::removePedido(unsigned cod, unsigned qnt)
{
    ...
    {
        if ((it->first) == cod)
        { // se o código digitado for encontrado no carrinho
            for (; Iit != _itens.end(); Iit++)
            { // vai ser procurado esse item na lista de itens
                if ((it->second.quantidade) == qnt)
                { // se a quantidade de itens a ser removido for igual a quantidade do carrinho
                ...
                {                                        // Quantidade a ser removida é menor que a do carrinho
                ...
                { // Quantidade a ser removida é maior que a do carrinho
...
compra: // referência para o goto
    ...
    while (erro)
    {
        ...
        std::cout << "Deseja finalizar a compra?"
                  << "  "
                  << "S/N" << std::endl;
        std::cin >> alt;
        ...
        case 'n':
            ...
            while (erro2)
            {
                std::cout << "Alterar o carrinho (alt)"
                          << "   "
                          << "Cancelar a compra (dlt)"
                          << "  "
                          << "Finalizar (end)" << std::endl;
                std::cin >> opc;
                if (opc == "alt")
                ...
                { // vai para a seção de compra finalizar o pedido
                    erro2 = !(Loja::compra());
                    erro = !(Loja::compra());
                }
                else
